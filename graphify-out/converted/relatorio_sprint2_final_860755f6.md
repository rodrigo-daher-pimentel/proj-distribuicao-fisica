<!-- converted from relatorio_sprint2_final.docx -->
...
Figura 8 – (reservada para diagrama do pipeline da Sprint 2, omitida nesta versão)
...
A Sprint 2, de 30/04 a 21/05/2026, estendeu o escopo metodológico ao implementar heurísticas construtivas adaptadas a frota heterogênea (Nearest Neighbor e Clarke-Wright Savings) e movimentos de busca local clássicos (2-opt intra-rota e Relocate inter-rota). Os objetivos acordados na Sprint Planning #2 foram comparar criteriosamente as duas heurísticas, quantificar o ganho marginal da busca local, identificar qual ponto de partida converge para o melhor ótimo local em cada instância e contrastar os resultados com o benchmark exato estabelecido na Sprint 1. Os entregáveis dessa sprint mapeiam-se aos itens 1.3.2, 1.3.3, 1.4.2, 1.5.3 e 1.5.4 da Estrutura Analítica do Projeto.
...
O restante do documento organiza-se em quatorze seções. A seção 2 apresenta a revisão da literatura, cobrindo distribuição física, VRP, CVRP, formulação MILP, eliminação de subtours, métodos exatos, heurísticas construtivas, busca local e metaheurísticas. A seção 3 descreve a base de dados da Prolog e a geração das instâncias de teste. A seção 4 formaliza o modelo matemático, incluindo a discussão crítica da limitação observada empiricamente na Sprint 2 (subseção 4.6). A seção 5 detalha a implementação em Pyomo. A seção 6 apresenta os resultados dos quatro experimentos computacionais com o método exato. A seção 7 discute a viabilidade do método exato. As seções 8 e 9 apresentam os resultados das heurísticas construtivas e da busca local desenvolvidas na Sprint 2. A seção 10 traz a análise comparativa consolidada entre exato, construtivas e refinadas por busca local. A seção 11 discute a viabilidade dos métodos heurísticos. As seções 12 e 13 tratam dos códigos Python e das ferramentas de gestão do projeto. A seção 14 traz as considerações finais, seguida pelas referências.
...
Uma segunda limitação da formulação, distinta da relaxação fraca do MTZ e não detectada nos experimentos da Sprint 1, foi revelada empiricamente pela Sprint 2: a restrição implícita de saída única por tipo de veículo, formalizada na nova subseção 4.6 a seguir.
...
Na Sprint 2, dois novos notebooks foram adicionados ao repositório. O notebook Aulas/7/Aula7_Heuristicas_Construtivas/notebook.ipynb implementa as heurísticas Nearest Neighbor e Clarke-Wright Savings com frota heterogênea e gera as oito soluções construtivas no critério total_cost. O notebook Aulas/8/Aula8_Busca_Local/notebook.ipynb consome essas soluções e aplica o pipeline 2-opt + Relocate, salvando as oito soluções refinadas em arquivos JSON que servem de entrada para as metaheurísticas da Sprint 3.
...
A predição apresentada nesta subseção foi confirmada empiricamente pela Sprint 2. A heurística Clarke-Wright, sem qualquer conhecimento da formulação MILP, identificou em C2 uma configuração com dois Fiorinos em paralelo (custo total R$ 712,06) que é R$ 41,98 mais barata do que a solução ótima do modelo MILP com um VUC (R$ 754,04). A discrepância confirma que o ponto de inflexão analítico estimado entre R$ 540 e R$ 545 é uma fronteira real do problema, e que a restrição implícita de saída única por tipo no modelo da Aula 4 (Seção 4.6) impede o MILP de acessá-la. Esse resultado é discutido em detalhe nas Seções 8.6, 8.8 e 10.2.
...
A Sprint 2 acrescentou dois notebooks adicionais. O notebook Aulas/7/Aula7_Heuristicas_Construtivas/notebook.ipynb implementa Nearest Neighbor e Clarke-Wright Savings com frota heterogênea e produz dezesseis soluções iniciais (combinações instância × heurística × critério), serializadas em JSON. O notebook Aulas/8/Aula8_Busca_Local/notebook.ipynb consome as oito soluções construtivas no critério total_cost e aplica em sequência os movimentos 2-opt e Relocate, salvando as oito soluções refinadas em arquivos JSON. Esses arquivos servem de entrada para as metaheurísticas previstas na Sprint 3.
...
A Sprint 2 incorporou ao cronograma os entregáveis 1.3.2 (Código de Heurísticas Construtivas), 1.3.3 (Código de Busca Local), 1.4.2 (Relatório Comparativo Heurísticas vs. Exato), 1.5.3 (Relatório Consolidado G1) e 1.5.4 (Apresentação Sprint Review #2), executados entre 30/04 e 23/05/2026 conforme as datas registradas no arquivo Cronograma.xlsx.
Quadro 4 – Datas dos entregáveis da Sprint 1
...
Como contribuição operacional concreta, a Sprint 2 formalizou um pipeline recomendado para a Prolog (Quadro 7): gerar soluções iniciais com Nearest Neighbor e Clarke-Wright em paralelo (custo combinado abaixo de 40 ms na maior instância), aplicar 2-opt e Relocate em ambas, e selecionar a de menor custo total. Os riscos identificados na Sprint 1 — aproximação Haversine, jornada validada em pós-processamento e amostra de um único dia — permanecem como itens de backlog para tratamento ao longo da Sprint 3 ou em extensões futuras. As soluções refinadas estão persistidas nos oito arquivos JSON da pasta Aulas/8/Aula8_Busca_Local/files/.
